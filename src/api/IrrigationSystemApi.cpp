...
void IrrigationSystemApi::setup()
{
  ...
  server->on("/status/pumps", HTTP_GET, [=]() -> void {
    Serial.println("Getting pump statuses...");
    Serial.println("Pump statuses are: " + String(systemStatus->getPumpStatuses()));
    server->send(200, "application/json", systemStatus->getPumpStatuses());
  });
